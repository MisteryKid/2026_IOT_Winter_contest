...
| 구분 | 시연 내용 | 링크 |
| :--- | :--- | :--- |
| **Hardware** | 센서 데이터 실시간 전송 (Arduino → Mobius) | [▶ 영상 보기](https://www.youtube.com/watch?v=rhVqj1J1fQg) |
| **Web UI** | Vue.js 기반 실시간 모니터링 및 데이터 시각화 | [▶ 영상 보기](https://www.youtube.com/watch?v=-rCXgiqqB3o) |
- 테스트 시나리오
...
<table table-borderless="true">
  <tr>
    ...
    <td align="center" width="33%">
      <img src="https://github.com/user-attachments/assets/ffd574c9-e368-40be-8648-f8fbea286551" width="80%"/><br>
      <sub><b>웹 이미지</b></sub>
    </td>
  </tr>
</table>
...
<table table-borderless="true">
  <tr>
    ...
    <td align="center" width="33%">
      <img src="https://github.com/user-attachments/assets/d8a80c38-a88c-426c-973d-5c76cdf450e6" width="80%"/><br>
      <sub><b>웹 이미지 </b></sub>
    </td>
  </tr>
</table>
...
## 한계점
1. **데이터 처리의 실시간성 부족** 
   - **현상**: 센서 데이터가 Mobius 플랫폼을 거쳐 웹 대시보드에 반영되기까지 수 초 이상의 딜레이가 발생함.

   - **원인**: 현재 HTTP REST API 기반의 폴링(Polling) 방식 또는 서버의 처리 속도 한계로 인해, '즉각적인 사고 예방'이라는 목적에 부합하는 ms 단위의 실시간성 확보가 어려움.

2. **센서 노이즈 및 측정 오차**
   - **현상**: 초음파 센서 특성상 운전자의 옷(바지) 재질이나 표면 각도에 따라 거리 값이 튀거나(Outlier) 부정확한 값이 측정됨.
   - **원인**: 하드웨어의 물리적 특성 및 차량 내 좁은 공간에서의 난반사 문제.

3. **사용자별 개인화 부재**
   - **현상**: 모든 운전자에게 동일한 거리 임계값(예: 15cm 이탈 시 경고)을 적용함.
   - **문제점**: 운전자마다 다리 길이, 시트 포지션, 선호하는 운전 자세가 다르기 때문에 일률적인 기준은 오작동의 원인이 될 수 있음.

4. **실제 차량 환경과의 괴리**
   - **현상**: 정지된 시뮬레이터 환경에서 테스트가 이루어짐.
   - **문제점**: 실제 주행 시 발생하는 노면 진동, 급커브 시의 몸 쏠림, 가속 시의 관성 등이 센서 값에 미치는 영향을 완벽히 반영하지 못함.

## 향후 과제 및 해결 방안
1. MQTT 프로토콜 도입을 통한 실시간성 강화
- 기존 HTTP REST 방식의 오버헤드를 줄이기 위해 MQTT(Publish/Subscribe) 기반 통신으로 전환.
- 기대 효과: 데이터 전송 지연을 ms 단위로 단축하여 긴박한 오조작 상황에서 즉각적인 피드백 가능.

2. 고도화된 필터링 알고리즘 개발
- 구상 중

3. 운전자 맞춤형 캘리브레이션 기능 추가
- 탑승 시 사용자의 신체 조건에 맞춘 '초기 자세 설정 모드' 도입.
- 기대 효과: 개인별 다리 길이와 시트 포지션에 최적화된 **동적 임계값**(Dynamic Threshold) 적용으로 범용성 확대.

4. 실제 주행 환경 기반의 데이터 보정
- 가속도 센서(IMU)를 추가하여 주행 중 발생하는 진동 및 원심력 데이터를 수집.
- 기대 효과: 신체 흔들림과 실제 자세 붕괴를 구분하는 로직을 구축하여 시스템의 신뢰도 향상.
