...
const int DELAY = 3000;              // 데이터 전송 주기 (10초)
const char* RVI = "2a";               // oneM2M 릴리즈 버전

// oneM2M 리소스 경로 설정
String CSEBASE = "/Mobius";           // CSE Base 이름
String R4_AE = "ae_Braker0124";
//String R4_AE = String("R4_TUTO_") + String(ORIGIN);  // Application Entity 이름
String DIS_CNT = "DIS";               // 온도 Container 이름
String PRE_CNT = "PRE";               // 습도 Container 이름
...
void loop() {
  ...
  float pressure = analogRead(FSR_PIN);
  ...
  Serial.print("cm | Press: "); Serial.print(pressure);
  Serial.println("[POST] Sending state to server...");
  
  postCIN(CSEBASE + "/" + R4_AE + "/" + DIS_CNT, String(distance));
  postCIN(CSEBASE + "/" + R4_AE + "/" + PRE_CNT, String(pressure));
...
void setDevice(){
  ...
  String DISPath = aePath + "/" + DIS_CNT;
  String PREPath = aePath + "/" + PRE_CNT;
  ...
  // 2. TEM CNT 확인 -> 없으면 생성
  int disSc = get(DISPath);
  if (disSc == 404 || disSc == 403) {
    Serial.println("[setDevice] DIS CNT not found -> creating...");
    int cr = postCNT(aePath, DIS_CNT);
    if (cr == 201 || cr == 200) {
      Serial.println("[setDevice] DIS CNT created");
    } else {
      Serial.println("[setDevice] DIS CNT create failed");
    }
  } else if (disSc == 200 || disSc == 201) {
    Serial.println("[setDevice] DIS CNT exists");
  }

  // 3. HUM CNT 확인 -> 없으면 생성
  int preSc = get(PREPath);
  if (preSc == 404 || preSc == 403) {
    Serial.println("[setDevice] PRE CNT not found -> creating...");
    int cr = postCNT(aePath, PRE_CNT);
    if (cr == 201 || cr == 200) {
      Serial.println("[setDevice] PRECNT created");
    } else {
      Serial.println("[setDevice] PRE CNT create failed");
    }
  } else if (preSc == 200 || preSc == 201) {
    Serial.println("[setDevice] PRE CNT exists");
  }
